class BandsController < ApplicationController
  ...
  def index
    if params[:query].present? && params[:query_1].present?
      @bands = Band.where("name LIKE ? AND idade::text LIKE ?", "%#{params[:query]}%", "%#{params[:query_1]}%").page(params[:page]).per(10)
    elsif params[:query].present?
      @bands = Band.where("name LIKE ?", "%#{params[:query]}%").page(params[:page]).per(10)
    elsif params[:query_1].present?
      @bands = Band.where("idade::text LIKE ?", "%#{params[:query_1]}%").page(params[:page]).per(10)
    else
      @bands = Band.all.page(params[:page]).per(10)
    end
